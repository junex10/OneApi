...
let AuthService = class AuthService {
    constructor(userModel, passwordResetModel, moduleModel, personModel, mailerService) {
        ...
        this.checkPermissions = async (permissions, code) => {
            const readPermissions = utils_1.JWTAuth.readToken(permissions);
            const auth = await (readPermissions === null || readPermissions === void 0 ? void 0 : readPermissions.permissions.filter((value) => code === value.actions.code));
            if (auth !== null) {
                if (auth.length > 0)
                    return true;
            }
    ...
    async createUser(request, file) {
        ...
        const user = await this.userModel.create({
            email: request.email,
            password: utils_1.Hash.makeSync(request.password),
            level_id: request.level_id || utils_1.Constants.LEVELS.USER,
            photo: file ? ('users/' + file.filename) : null,
            token: await this.generateURL(),
            verified: utils_1.Constants.USER.USER_VERIFIED.NO_VERIFIED,
            status: utils_1.Constants.USER.USER_VERIFIED.NO_VERIFIED
        });
